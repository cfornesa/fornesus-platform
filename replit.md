...
- **Canonical Origin Resolution**: Feed, export, immersive embed, and piece embed URL generation goes through `getCanonicalOrigin()`. The precedence is the first `ALLOWED_ORIGINS` entry, then `PUBLIC_SITE_URL`, then `x-forwarded-proto`/`x-forwarded-host` or the raw Express host, then `https://meet.fornesus.com` as the final fallback. In production, set `ALLOWED_ORIGINS` with the primary public origin first so generated URLs are stable.
...
- **Interactive Pieces**: Owners can generate and manage reusable `p5`, `c2`, and `three` pieces. AI generation returns mandatory HTML/CSS/JS code blocks, the API preflights drafts before display, and embed snippets resolve the current piece version live at `/embed/pieces/:id`. Piece runtime assets are served from `/api/runtimes/*` to avoid Replit proxy interception.
- **Local Media Library**: Owners can upload or import images into MySQL-backed `media_assets`, edit title/alt text, select featured images, and reuse assets in rich posts and exhibits.
- **Immersive Views**: Images, art pieces, and exhibit walls have dedicated immersive routes (`/immersive/images/:encodedRef`, `/immersive/pieces/:id`, `/immersive/exhibits/:slug`) plus iframe embed modes. Current working-tree changes improve fullscreen locking, visual viewport sizing, and camera preservation on resize.
- **Exhibits**: Owners can build exhibit walls from art pieces and media assets. Exhibits have slugs, descriptions, optional artist statement/biography, and configurable row/column layout.
...
- **POSSE Platforms**: Owner-authored posts can syndicate to WordPress.com, self-hosted WordPress, Medium, Blogger, Substack, Bluesky, LinkedIn, Facebook, and Instagram. Draft and scheduled posts can retain selected platform IDs and per-platform social captions until publication.
